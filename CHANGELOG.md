...
Unreleased
----------
* Fix: `Reachable` is `unknown`, not `no`, when the server refused us rather than the address. `client.Rcpt`'s error was discarded, so a blocklisted IP and a missing mailbox were the same to us [#232](https://github.com/AfterShip/email-verifier/pull/232)
